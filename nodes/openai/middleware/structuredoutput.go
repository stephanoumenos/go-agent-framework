...
func structuredOutputMiddleware[SOut any](ctx heart.ResolverContext, in openai.ChatCompletionRequest, next heart.NodeDefinition[openai.ChatCompletionRequest, openai.ChatCompletionResponse]) (SOut, error) {
	var sOut SOut
	...
	out, err := next.Bind(heart.Into(in)).Out(ctx)
	if err != nil {
